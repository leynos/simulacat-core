...
const commitUrlBuilders = {
  url: (commit, baseUrls) =>
    apiUrl(baseUrls, `/repos/${repositoryPath(commit.owner, commit.repo)}/commits/${urlPathSegment(commit.sha)}`),
  html_url: (commit, baseUrls) =>
    webUrl(baseUrls, `/${repositoryPath(commit.owner, commit.repo)}/commit/${urlPathSegment(commit.sha)}`)
...
const commitObjectUrl = (commit: Pick<GitHubCommit, 'owner' | 'repo'>, sha: string, baseUrls: BaseUrls) =>
  apiUrl(baseUrls, `/repos/${repositoryPath(commit.owner, commit.repo)}/git/commits/${urlPathSegment(sha)}`);
...
const commitResourceUrl = (commit: Pick<GitHubCommit, 'owner' | 'repo'>, sha: string, baseUrls: BaseUrls) =>
  apiUrl(baseUrls, `/repos/${repositoryPath(commit.owner, commit.repo)}/commits/${urlPathSegment(sha)}`);
...
export const projectCommitUrls = (commit: CommitUrlPayload, baseUrls: BaseUrls): CommitUrlPayload => {
  ...
  const tree = {
    ...
    url:
      ...
      apiUrl(
        baseUrls,
        `/repos/${repositoryPath(commit.owner, commit.repo)}/git/trees/${urlPathSegment(projected.commit.tree.sha)}`
      )
  };
