...
describe('actor context helpers', () => {
  ...
  it('resolves all supported actor kinds against the simulation store', () => {
    const user = resolveActorContext(simulationStore, buildActorContext(headers({[requestActorHeader]: 'user:dev'})));
    const app = resolveActorContext(simulationStore, buildActorContext(headers({[requestActorHeader]: 'app:2000'})));
    const installation = resolveActorContext(
      simulationStore,
      buildActorContext(headers({[requestActorHeader]: 'installation:2000'}))
    );
    const anonymous = resolveActorContext(simulationStore, buildActorContext(headers({})));

    expect(user.user?.login).toBe('dev');
  ...
  it('selects a required user actor from a REST request', () => {
    const result = requireUserActor(
      {transport: 'rest', request: requestWithActor('user:reviewer'), surface: 'GET /user'},
      simulationStore
    );
  ...
  it('records authentication failure for unauthenticated REST requests', () => {
    const result = requireUserActor(
      {transport: 'rest', request: requestWithActor(undefined), surface: 'GET /user'},
      simulationStore
    );
